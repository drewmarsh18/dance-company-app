...
function statusFg(s: string | null) {
  const l = (s ?? "").toLowerCase()
  if (l === "confirmed") return COLORS.primary
  if (l.startsWith("cancelled")) return COLORS.red
  return COLORS.textMuted
}
function memberStatusInfo(member: AdminMember, memberPlans: MemberPlan[], memberBookings: AdminBooking[]) {
  ...
  const recentBooking = memberBookings.some(
...
export default function AdminMembersScreen() {
  ...
  function memberBookings(m: AdminMember) {
    return bookings.filter((b) => b.userId === m.userId || b.clientEmail.toLowerCase() === m.email.toLowerCase())
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      <FlatList
        ...
        renderItem={({ item: m }) => {
          const mPlans = memberPlans(m)
          const mBookings = memberBookings(m)
          const credits = creditsFor(m)
          const statusInfo = memberStatusInfo({ ...m, creditsRemaining: credits }, mPlans, mBookings)
          ...
          return (
            <MemberCard
              ...
              isOpen={isOpen}
              onToggle={() => setExpanded(isOpen ? null : m.id)}
              onCreditsChange={(id, val) => setLocalCredits((prev) => ({ ...prev, [id]: val }))}
              onPlanAdded={(plan, creditsAdded) => {
                setLocalPlans((prev) => [plan, ...(prev ?? plans)])
                setLocalCredits((prev) => ({ ...prev, [m.id]: credits + creditsAdded }))
              }}
              onPlanRemoved={(planId, deducted) => {
                setLocalPlans((prev) => (prev ?? plans).filter((p) => p.id !== planId))
                setLocalCredits((prev) => ({ ...prev, [m.id]: Math.max(0, credits - deducted) }))
              }}
            />
          )
        }}
      ...
      <Modal visible={showAddForm} animationType="slide" presentationStyle="pageSheet">
        <AddMemberForm
          ...
          onSuccess={(member) => {
            setLocalMembers([member, ...members])
...
type MemberCardProps = {
  ...
  statusInfo: { label: string; bg: string; fg: string }
  isOpen: boolean
  onToggle: () => void
...
function MemberCard({ member: m, credits, plans, bookings, packages, statusInfo, isOpen, onToggle, onCreditsChange, onPlanAdded, onPlanRemoved }: MemberCardProps) {
  ...
  async function addSingleSession(label: string) {
    ...
    const { data, error } = await authClient.$fetch(`${API}/api/admin/members/plans`, {
      method: "POST",
      body: JSON.stringify({ memberId: m.id, userId: m.userId, email: m.email, currentCredits: credits, label }),
  ...
  async function assignPackage() {
    ...
    const { data, error } = await authClient.$fetch(`${API}/api/admin/members/plans`, {
      method: "POST",
      body: JSON.stringify({ memberId: m.id, userId: m.userId, email: m.email, currentCredits: credits, packageId: selectedPkg }),
      headers: { "Content-Type": "application/json" },
    })
    setSaving(false)
    ...
    onPlanAdded((data as any).plan, pkg.sessions)
  }

  async function removePlan(plan: MemberPlan) {
    Alert.alert(
      "Remove Plan",
      `Remove "${plan.planName}" from ${m.name || m.email}? This will deduct ${plan.sessions} credits.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            const { error } = await authClient.$fetch(`${API}/api/admin/members/plans`, {
              method: "DELETE",
              body: JSON.stringify({ planId: plan.id, memberId: m.id, planSessions: plan.sessions, currentCredits: credits }),
              headers: { "Content-Type": "application/json" },
            })
            setSaving(false)
  ...
  async function setCredits() {
    ...
    const { error } = await authClient.$fetch(`${API}/api/admin/members/credits`, {
      method: "POST",
      body: JSON.stringify({ memberId: m.id, newCredits: val }),
      headers: { "Content-Type": "application/json" },
    })
    setSaving(false)
    if (error) { Alert.alert("Error", "Failed to set credits."); return }
    onCreditsChange(m.id, val)
    setEditCredits("")
  }
        ...
        <View style={styles.expanded}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>Assign a plan</Text>
            </View>
            ...
            <TouchableOpacity
              style={[styles.btnPrimary, (!selectedPkg || saving) && styles.btnDisabled]}
              onPress={assignPackage}
              disabled={!selectedPkg || saving}
              activeOpacity={0.7}
            >
                    ...
                    <View style={styles.planRow}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.planMeta}>
                          {plan.sessions} {plan.sessions === 1 ? "credit" : "credits"} · ${plan.pricePaid}
                          {purchaseDate ? ` · ${purchaseDate}` : ""}
                        </Text>
                        {expiryDate ? <Text style={styles.planMeta}>Expires {expiryDate}</Text> : null}
                      </View>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                        <View style={[styles.badge, { backgroundColor: planStatusBg }]}>
                          <Text style={[styles.badgeText, { color: planStatusFg }]}>{s}</Text>
                        </View>
                        {s !== "Used" && (
                          <TouchableOpacity onPress={() => removePlan(plan)} disabled={saving} hitSlop={8}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.inputRow}>
              ...
              />
              <TouchableOpacity
                style={[styles.btnOutline, (!editCredits || saving) && styles.btnDisabled]}
                onPress={setCredits}
                disabled={!editCredits || saving}
                activeOpacity={0.7}
              >
        ...
        <TouchableOpacity style={styles.overlay} onPress={() => setPkgPickerOpen(false)} activeOpacity={1}>
          <View style={styles.sheet}>
            ...
            {packages.map((pkg) => (
              <TouchableOpacity
                key={pkg.id}
                style={[styles.sheetOption, selectedPkg === pkg.id && { backgroundColor: COLORS.primaryLight }]}
                onPress={() => { setSelectedPkg(pkg.id); setPkgPickerOpen(false) }}
                activeOpacity={0.7}
              >
                <Text style={[styles.sheetOptionText, selectedPkg === pkg.id && { color: COLORS.primary }]}>
...
function AddMemberForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: (m: AdminMember) => void }) {
  ...
  async function submit() {
    ...
    const { data, error } = await authClient.$fetch(`${API}/api/admin/members`, {
      method: "POST",
      body: JSON.stringify({ name: name.trim(), email: email.trim(), phone: phone.trim(), goals: goals.trim(), creditsRemaining: parseInt(credits, 10) || 0 }),
      headers: { "Content-Type": "application/json" },
    })
    setSaving(false)
