...
function slotsForDate(dateIso: string, week: DayAvailability[]): string[] {
  ...
  const start = Number(config.startTime.split(":")[0])
  const end = Number(config.endTime.split(":")[0])
  const slots: string[] = []
              ...
              >
                <View style={styles.coachAvatar}>
                  <Text style={styles.coachAvatarText}>
                    {coach.name.trim().split(/\s+/).filter(Boolean).map((w: string) => w[0]).slice(0, 2).join("").toUpperCase()}
                  </Text>
                </View>
                ...
                {!coach.hasAvailability ? (
                  <View style={styles.unavailBadge}>
                    <Text style={styles.unavailText}>No availability</Text>
                  </View>
                ) : (
...
function BookingStep({
  ...
  const activePlans = useMemo(
    () => plans.filter((p) => planDisplayStatus(p) === "Active"),
    [plans],
  )
  const [selectedPlanId, setSelectedPlanId] = useState<string | null>(
    activePlans.length === 1 ? activePlans[0].id : null,
  )
  ...
  const weekSlots = useMemo(() => {
    return weekDays.map((d) => {
      ...
      const slots = isPast ? [] : slotsForDate(iso, detail.week)
  ...
  function handleConfirm() {
    ...
    onConfirm({
      ...
      planSessions: effectivePlan?.sessions,
      sessionType: effectivePlan ? planSessionType(effectivePlan.planName) : "pack-hour",
    })
  }
            ...
            {weekSlots.map(({ date, iso, slots, taken, isPast }) => {
              ...
              return (
                <View
                  ...
                  style={[
                    styles.calCol,
                    (isPast || slots.length === 0) && { opacity: 0.35 },
                    isSelected && styles.calColSelected,
                  ]}
                >
                  ...
                  {slots.length === 0 ? (
                    <Text style={styles.calNoSlot}>—</Text>
                  ) : (
                    slots.map((slot) => {
                      const isTaken = taken.has(slot)
                      const isSlotSel = isSelected && selectedTime === slot
                      return (
                        <TouchableOpacity
                          key={slot}
                          style={[
                            styles.slotBtn,
                            isSlotSel && styles.slotBtnSelected,
                            isTaken && styles.slotBtnTaken,
                          ]}
                          onPress={() => { setSelectedDate(iso); setSelectedTime(slot) }}
        ...
        <View style={styles.confirmBar}>
          {selectedDate && selectedTime ? (
            <View style={{ flex: 1 }}>
              <Text style={styles.confirmDate}>
                {new Date(`${selectedDate}T00:00:00`).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })} · {selectedTime}
              </Text>
          ...
          >
            {confirming
              ...
              : <><Check size={16} color="#fff" /><Text style={styles.confirmBtnText}>Confirm</Text></>
            }
          </TouchableOpacity>
        </View>
      </ScrollView>
...
export default function BookScreen() {
  ...
  const loadCoaches = useCallback(async () => {
    try {
      ...
      const d = data as { coaches: Coach[] }
      setCoaches(d.coaches.filter((c) => c.hasAvailability))
    } catch (e) {
      Alert.alert("Error", e instanceof Error ? e.message : "Could not load prep masters.")
    } finally {
  ...
  const handleSelectCoach = useCallback(async (coach: Coach) => {
    ...
    try {
      ...
      setDetail(data as CoachDetail)
      setStep("booking")
    } catch (e) {
      Alert.alert("Error", e instanceof Error ? e.message : "Could not load availability.")
    } finally {
  ...
  const handleConfirm = useCallback(async (args: {
    ...
    try {
      const { data, error } = await authClient.$fetch(`${API_BASE}/api/booking/create`, {
        method: "POST",
        body: JSON.stringify({
          prepMasterId: selectedCoach.id,
          prepMasterName: selectedCoach.name,
          date: args.date,
          time: args.time,
          notes: args.notes,
          planId: args.planId,
          planSessions: args.planSessions,
      ...
      if (!result.ok) {
        if (result.error === "NO_CREDITS") {
          ...
          Alert.alert("Could not book", result.error ?? "Please try again.")
        }
        return
      }
      Alert.alert("Session requested!", `Your request with ${selectedCoach.name} on ${args.date} at ${args.time} has been sent. 1 credit used.`, [
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      <View style={styles.header}>
        <TouchableOpacity
          ...
          onPress={() => {
            if (step === "booking") { setStep("coaches"); setDetail(null) }
            else router.back()
          }}
        >
      ...
      {coachesLoading ? (
        ...
        <BookingStep
          ...
          plans={plans}
          credits={credits}
          onBack={() => { setStep("coaches"); setDetail(null) }}
...
const styles = StyleSheet.create({
  ...
  uniChip: {
    ...
    borderRadius: RADIUS.full, backgroundColor: COLORS.grayLight,
  },
  uniChipText: { fontSize: 11, fontWeight: "600", color: COLORS.textMuted },
  unavailBadge: {
    backgroundColor: COLORS.grayLight, paddingHorizontal: 8, paddingVertical: 3, borderRadius: RADIUS.full,
  },
  ...
  calCol: {
    ...
    backgroundColor: COLORS.surface, padding: 4, gap: 4, alignItems: "center",
  },
  ...
  slotBtn: {
    width: "100%", borderRadius: 4, paddingVertical: 5,
