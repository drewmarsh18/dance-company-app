...
function statusColors(status: string) {
  ...
  if (s === "pending") return { bg: COLORS.amberLight, text: COLORS.amber }
  ...
  async function callAction(action: "confirm" | "decline" | "edit") {
    ...
    try {
      ...
      const { data, error } = await authClient.$fetch(
        `${API_BASE}/api/portal/bookings/${booking.id}`,
      ...
      if (action === "decline") { setStatus("Cancelled"); onUpdate(booking.id, { status: "Cancelled" }) }
      if (action === "edit") {
          ...
          <View style={styles.cardDateRow}>
            ...
            {localTime ? (
              <>
                ...
                <Text style={styles.cardTime}>{localTime}</Text>
              </>
            ) : null}
          </View>
        ...
        <View style={styles.cardBody}>
            ...
            <View style={{ gap: 4 }}>
              {booking.dancerEmail ? (
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText}>{booking.dancerEmail}</Text>
                </View>
              ) : null}
              {booking.dancerPhone ? (
                <View style={styles.infoRow}>
                  <Phone size={13} color={COLORS.textMuted} />
                  <Text style={styles.infoText}>{booking.dancerPhone}</Text>
                </View>
              ) : null}
            </View>
          )}
          ...
          {localNotes && mode === "idle" ? (
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>{localNotes}</Text>
            </View>
          ) : null}
              ...
              {isPending && (
                <>
                  <TouchableOpacity
                    ...
                    onPress={() => callAction("confirm")}
                    disabled={saving}
                    activeOpacity={0.8}
                  >
                    {saving ? <ActivityIndicator size="small" color="#fff" /> : (
                      <><Check size={14} color="#fff" /><Text style={styles.actionBtnPrimaryText}>Confirm</Text></>
                    )}
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.actionBtn, styles.actionBtnDanger, saving && { opacity: 0.5 }]}
                    onPress={() => Alert.alert("Decline booking?", "This will cancel the session.", [
                      { text: "Cancel", style: "cancel" },
                      { text: "Decline", style: "destructive", onPress: () => callAction("decline") },
                    ])}
                    disabled={saving}
                    activeOpacity={0.8}
                  >
                    <X size={14} color={COLORS.red} />
                    <Text style={styles.actionBtnDangerText}>Decline</Text>
                  </TouchableOpacity>
                </>
              )}
              <TouchableOpacity
          ...
          {mode === "edit" && (
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined}>
              <View style={styles.editPanel}>
                ...
                <TextInput
                  style={[styles.editInput, { minHeight: 64, textAlignVertical: "top" }]}
                  value={editNotes}
                ...
                <View style={styles.actionRow}>
                  ...
                  <TouchableOpacity
                    style={[styles.actionBtn, styles.actionBtnGhost]}
                    onPress={() => { setEditDate(localDate); setEditTime(localTime); setEditNotes(localNotes); setMode("idle") }}
                    activeOpacity={0.8}
                  >
        ...
        <View style={styles.section}>
          ...
          {(data?.upcoming ?? []).length === 0 ? (
            <View style={styles.emptyCard}>
              <Text style={styles.emptyText}>No upcoming sessions booked yet.</Text>
            </View>
          ) : (
            (data?.upcoming ?? []).map((b) => (
        ...
        {(data?.completed ?? []).length > 0 && (
          <View style={styles.section}>
            ...
            {(data?.completed ?? []).map((b) => (
              <BookingCard key={b.id} booking={b} dimmed onUpdate={handleUpdate} />
            ))}
          </View>
        )}
